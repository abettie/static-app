...
| 環境 | S3バケット | リージョン |
|------|-----------|-----------|
| ステージング | `terraform-state-stg-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066` | `ap-northeast-1` |
| 本番 | `terraform-state-prod-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066` | `ap-northeast-1` |
...
export AWS_PROFILE=static-stg && aws s3api create-bucket \
  --bucket terraform-state-stg-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066 \
  --region ap-northeast-1 \
  --create-bucket-configuration LocationConstraint=ap-northeast-1

export AWS_PROFILE=static-stg && aws s3api put-bucket-versioning \
  --bucket terraform-state-stg-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066 \
  --versioning-configuration Status=Enabled

# 本番環境用
aws sso login --profile static-prod
export AWS_PROFILE=static-prod && aws s3api create-bucket \
  --bucket terraform-state-prod-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066 \
  --region ap-northeast-1 \
  --create-bucket-configuration LocationConstraint=ap-northeast-1

export AWS_PROFILE=static-prod && aws s3api put-bucket-versioning \
  --bucket terraform-state-prod-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066 \
  --versioning-configuration Status=Enabled
```
...
`application/`ディレクトリの内容をS3バケットに同期します。

#### ステージング環境
```bash
cd $(git rev-parse --show-toplevel) && export AWS_PROFILE=static-stg && aws s3 sync ./application/ s3://apps-stg-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066/
```

#### 本番環境
```bash
cd $(git rev-parse --show-toplevel) && export AWS_PROFILE=static-prod && aws s3 sync ./application/ s3://apps-prod-675f09ae-9bb8-4d10-b5f2-77c2f1bb1066/
```
